...
// ==========================================
// COMPONENT: Sighting Card (For Image Search Results)
// ==========================================
...
// ==========================================
// COMPONENT: Timeline Card (For ID Search Results)
// ==========================================
const TimelineCard = ({ data }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 flex items-center shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex-1">
        <div className="flex items-center text-sm text-slate-500 space-x-4">
          <div className="flex items-center font-medium text-slate-700"><Camera className="w-3.5 h-3.5 mr-1.5 text-blue-500" /> {data.camera}</div>
          <div className="flex items-center"><Clock className="w-3.5 h-3.5 mr-1.5" /> {data.timestamp}</div>
        </div>
      </div>
    </div>
  );
...
const EventFeedView = () => {
  ...
  useEffect(() => {
    ...
    const logInterval = setInterval(() => {
      ...
      setLogs(prev => [{ time: timestamp, ...randomEvent }, ...prev].slice(0, 50));
    }, 2500);
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-6">
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-700 overflow-hidden shadow-xl">
        ...
        {/* Log Entries */}
        <div className="p-4 max-h-[600px] overflow-y-auto space-y-1 font-mono text-sm" style={{ scrollbarWidth: 'thin', scrollbarColor: '#475569 transparent' }}>
          {logs.map((log, index) => (
            <div 
              key={index} 
              ...
              style={{ animation: index === 0 ? 'fadeIn 0.3s ease-out' : 'none' }}
            >
...
// ==========================================
// COMPONENT: Investigator View (with IMG + ID Search tabs)
// ==========================================
const InvestigatorView = () => {
  ...
  const handleIdSearch = async () => {
    if (!searchId.trim()) {
      setIdSearchError("Please enter a person ID.");
      return;
    }
              ...
              {idSearchResults.length > 0 ? (
                <>
                  ...
                  <div className="space-y-3">
                    {idSearchResults.map((entry, idx) => (
                      <TimelineCard key={idx} data={entry} />
                    ))}
                  </div>
                </>
              ) : (
